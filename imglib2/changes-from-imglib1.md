...
At the [2011 hackathon in Madison](/events/2011-hackathon-in-madison), quite a lot was done about design issues of the originally published ImgLib (which was already the 6th generation). Unfortunately, these improvements were not possible in a fully backwards-compatible manner.
...
-   The `ImgOpener` returns an `ImgPlus`, not an `Img` to store additional meta-data as retrieved through Bio-Formats.
